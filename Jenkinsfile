        ...
        stages {
          ...
          stage('Build') {
            ...
            steps {
              ...
              dir('tmp') {
                timestamps {
                  sh '../scripts/firedrake-install $COMPLEX --tinyasm --disable-ssh --minimal-petsc --slepc --documentation-dependencies --install thetis --install gusto --install icepack --install irksome --no-package-manager --package-branch tsfc dual_base_fiat --package-branch FInAT dual_base_fiat --package-branch ufl ReubenHill/new-el --package-branch fiat ReubenHill/new-el || (cat firedrake-install.log && /bin/false)'
                }
              }
            }
          }
